...
def _runtime_receipt(path: Path, registry: Path, eligibility: Path) -> Path:
    roots = {
...
def _release_dir(tmp_path: Path) -> tuple[Path, Path, Path, Path]:
    ...
    receipt = _runtime_receipt(tmp_path / "receipt.json", registry, eligibility)
    ...
    return release_dir, receipt, registry, eligibility


def _verify_release(release_dir: Path, receipt: Path, registry: Path, eligibility: Path, output: Path) -> subprocess.CompletedProcess[str]:
...
def test_release_phase_binds_central_directory_to_receipt_registry_eligibility_and_runtime_roots(tmp_path: Path) -> None:
    release_dir, receipt, registry, eligibility = _release_dir(tmp_path)
