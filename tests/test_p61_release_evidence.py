from __future__ import annotations

from pathlib import Path

SECRET_MARKERS = ("sk_live_", "xoxb-", "ghp_", "sntrys_", "BEGIN PRIVATE KEY", "prod-token", "nvapi-", "actual-secret-value")


def test_p61_docs_and_verify_are_wired() -> None:
    release = Path("docs/release-evidence.md").read_text(encoding="utf-8")
    verify = Path("scripts/verify.sh").read_text(encoding="utf-8")
    roadmap = Path("docs/operations/p61-ticket-roadmap.md").read_text(encoding="utf-8")
    summary = Path("docs/operations/p61-final-summary.md").read_text(encoding="utf-8")
    for required in ["P61-001", "P61-002", "P61-003", "P61-004", "P61-005", "P61-006", "P61-007", "P61-008"]:
        assert required in roadmap
        assert required in summary
    for required in [
        "P61 Local Shadow Connector Validation Evidence",
        "evals/shadow/p61_local_shadow_source.json",
        "app/services/local_shadow_connector_validation.py",
        "scripts/run_local_shadow_connector_validation.py",
        "tests/test_local_shadow_connector_validation.py",
        "/tmp/opscat-local-shadow-connector-validation-latest.md",
    ]:
        assert required in release
    assert "local_shadow_connector_validation_smoke" in verify
    assert all(marker not in release + roadmap + summary for marker in SECRET_MARKERS)
