...
def build_llm_judgment_prompt_messages(context_packet: Mapping[str, Any]) -> list[dict[str, str]]:
    ...
    system = (
        ...
        "Use local_mock_auto_allowed only when there is sufficient evidence, no missing_evidence, "
        "all safe_actions are read-only mock.* actions, and there is no rollback, restart, no-data, stale-data, or metric-only ambiguity. "
        "If rollback, restart, production-impacting action, no-data ambiguity, missing evidence, or risky operational mutation is present, "
        "choose approval_required or human_required and list the risky action in forbidden_actions_detected. "
