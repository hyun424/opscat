...
RUNBOOKS: tuple[Runbook, ...] = (
    Runbook(
        ...
        steps=(
            ...
            RunbookStep("draft rollback", "mock.create_rollback_pr", ("bad_deploy_identified", "rollback_plan_present"), "mock:pull_requests:write", "medium", True, "rollback PR remains draft/mock", "mock recovery check passes"),
        ),
    ),
    Runbook(
        key="api_5xx_spike",
        ...
        steps=(
            ...
            RunbookStep("open tracking ticket", "mock.create_incident_ticket", ("evidence_cited", "incident_summary_present"), "mock:tickets:write", "low", True, "mock ticket only", "ticket_id_recorded"),
        ),
    ),
    Runbook(
        key="queue_backlog",
        ...
        steps=(
            RunbookStep("collect worker context", "mock.get_error_context", ("incident_summary_present",), "mock:context:read", "read_only", True, "none", "queue metrics collected"),
            RunbookStep("restart mock worker", "mock.execute_restart_worker", ("worker_target_confirmed",), "mock:workers:restart", "low", True, "single mock worker restart", "mock.verify_recovery"),
        ),
    ),
    Runbook(
        ...
        steps=(
            RunbookStep("check connector health", "mock.get_runbook", ("connector_id_present",), "mock:runbooks:read", "read_only", True, "none", "connector health known"),
        ),
    ),
    Runbook(
        key="diagnostic_only",
        ...
        steps=(
            RunbookStep("collect context only", "mock.get_error_context", ("incident_summary_present",), "mock:context:read", "read_only", True, "none", "evidence package created"),
        ),
    ),
)
...
def select_runbook(incident: Incident, candidates: list[RootCauseCandidate]) -> Runbook:
    haystack = " ".join([incident.service, incident.environment, incident.summary or "", incident.root_cause_candidate or "", str(incident.alert_payload), *(candidate.hypothesis for candidate in candidates[:2])]).lower()
