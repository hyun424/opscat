...
from app.services.p145_runner import p145_release_case_catalog, run_p145_preliminary_matrix

ROOT = Path(__file__).resolve().parents[1]
DEFAULT_PROFILE = ROOT / "evals/p145/input/response-duty-profile.json"
DEFAULT_OUTPUT = ROOT / "evals/p145/output"
DEFAULT_REVIEW = ROOT / "evals/p145/final-implementation-review.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if profile.get("cases") != p145_release_case_catalog():
            raise ValueError("p145_profile_catalog_drift")
        args.output_dir.mkdir(parents=True, exist_ok=True)
        evidence_path = args.output_dir / "release-evidence.json"
        if args.mode == "preliminary":
            matrix = run_p145_preliminary_matrix(ROOT, output_dir=args.output_dir / "selector-runs")
            manifest = build_p145_freeze_manifest(project_root=ROOT, profile=profile, matrix=matrix)
            evidence = build_p145_preliminary_evidence(matrix, manifest)
            ...
            evidence = assemble_p145_final_evidence(
            ...
            if args.canonical_matrix.read_bytes() != matrix_before or args.freeze_manifest.read_bytes() != manifest_before:
                raise ValueError("p145_final_mode_rewrote_frozen_input")
            _write_json(evidence_path, evidence)
        result = {
            "mode": args.mode,
        ...
        return 0 if args.mode == "preliminary" or evidence["status"] == P145_READY_STATUS else 1
    except Exception as exc:
        print(json.dumps({"status": P145_BLOCKED_STATUS, "error": str(exc), "error_type": type(exc).__name__}, sort_keys=True, separators=(",", ":")), file=sys.stderr)
        return 1
