...
1. **P171-001 — Sealed episode schema.** Store agent-visible evidence separately
   from operator-confirmed labels, commit predictions through a preregistered
   hash, bind the sealed case-ID set, and prevent future-truth leakage across
   root cause, affected service, precursor onset, severity, response class,
   natural recovery, uncertainty, and citations.
...
3. **P171-003 — Sample gate semantics.** Produce only informational point
   estimates below 30 labeled incident/precursor episodes or 300 healthy
