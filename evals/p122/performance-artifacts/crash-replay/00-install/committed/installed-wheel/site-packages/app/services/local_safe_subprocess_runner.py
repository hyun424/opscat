...
class ActualLocalSubprocessTransport:
    """Fail-closed placeholder; shipped package does not execute arbitrary command strings."""
    ...
    def run(self, command: str, *, ticket_id: str, timeout_seconds: int, artifact_dir: Path) -> LocalRunRecord:
        ...
        paths["stderr"].write_text("actual local subprocess transport is excluded from shipped package execution\n", encoding="utf-8")
        return LocalRunRecord(
            ticket_id=ticket_id,
            command=command,
            status="failed_closed",
            exit_code=126,
            reason="actual local subprocess transport is excluded from shipped package execution",
            stdout_path=paths["stdout"],
            stderr_path=paths["stderr"],
        )
