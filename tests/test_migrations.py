...
def test_run_migrations_creates_current_schema_and_records_version(tmp_path) -> None:  # type: ignore[no-untyped-def]
    ...
    assert applied == ["0001_initial", "0002_identity_memberships", "0003_audit_events", "0004_secret_records"]
    ...
    assert [row[0] for row in rows] == ["0001_initial", "0002_identity_memberships", "0003_audit_events", "0004_secret_records"]
...
def test_run_migrations_is_idempotent(tmp_path) -> None:  # type: ignore[no-untyped-def]
    ...
    assert [(item.version, item.applied) for item in status] == [("0001_initial", True), ("0002_identity_memberships", True), ("0003_audit_events", True), ("0004_secret_records", True)]
    with engine.connect() as connection:
        rows = connection.execute(select(schema_migrations.c.version)).all()
    assert len(rows) == 4
