#!/usr/bin/env python3
"""Run and freeze the exact 32-case P140 qualification."""
...
from app.services.p140_release_evidence import (  # noqa: E402
    P140_READY_STATUS,
    assemble_p140_final_evidence,
    build_p140_freeze_manifest,
    build_p140_preliminary_evidence,
)
from app.services.p140_runner import p140_release_case_catalog, run_p140_preliminary_matrix  # noqa: E402

DEFAULT_PROFILE = ROOT / "evals/p140/input/p139-deadman-adapter-profile.json"
DEFAULT_OUTPUT = ROOT / "evals/p140/output"
DEFAULT_REVIEW = ROOT / "evals/p140/final-implementation-review.json"
...
def main() -> int:
    ...
    try:
        ...
        if profile.get("cases") != p140_release_case_catalog():
        ...
        if args.mode == "preliminary":
            matrix = run_p140_preliminary_matrix(ROOT)
            manifest = build_p140_freeze_manifest(project_root=ROOT, profile=profile, matrix=matrix)
            evidence = build_p140_preliminary_evidence(matrix, manifest)
            ...
            if matrix_path.read_bytes() != matrix_before or manifest_path.read_bytes() != manifest_before:
                raise ValueError("p140_final_mode_rewrote_frozen_input")
            _write_json(evidence_path, evidence)
        ...
        return 0 if args.mode == "preliminary" or evidence["status"] == P140_READY_STATUS else 1
    except Exception as exc:
        print(json.dumps({"release_status": "p140_blocked", "error": str(exc), "error_type": type(exc).__name__}, sort_keys=True, separators=(",", ":")), file=sys.stderr)
        return 1
