#!/usr/bin/env python3
"""Generate P147 preliminary or final qualification artifacts."""
...
from app.services.p147_durable_shadow import (  # noqa: E402
    P147_CONTRACT,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.mode == "preliminary":
            artifacts = generate_p147_preliminary_artifacts(project_root=args.project_root, output_dir=args.output_dir)
            _print_receipt("preliminary", artifacts)
            return 0
        evidence_path = generate_p147_release_artifact(
        ...
        print(json.dumps({"phase": "p147", "status": "blocked", "error_type": type(exc).__name__, "error": str(exc)}, sort_keys=True, separators=(",", ":")), file=sys.stderr)
        return 1
