...
def test_provider_signals_for_same_outage_correlate_once_with_evidence() -> None:
    ...
    signals = [
        CorrelationSignal(provider="sentry", idempotency_key="s1", workspace_id="alpha", service="payment-api", environment="staging", fingerprint="pay-timeout", occurred_at=ts, deploy_marker="v1.42"),
        CorrelationSignal(provider="datadog", idempotency_key="d1", workspace_id="alpha", service="payment-api", environment="staging", fingerprint="pay-timeout", occurred_at=ts, deploy_marker="v1.42"),
