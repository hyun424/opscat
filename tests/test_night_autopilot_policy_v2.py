...
def test_night_autopilot_v2_requires_confidence_blast_radius_simulation_and_memory() -> None:
    ...
    base = PolicyContext(capabilities=default_capabilities({"mock:workers:restart"}), service="worker", environment="staging", night_autopilot=True, confidence=0.91, simulation_passed=True, blast_radius_scope="service", rollback_available=True)
