...
| secret exposure in evidence or report export | Evidence/report links could leak tokens, emails, or provider secret values. | Existing redaction is used for reports; docs tests assert no known secret markers. |
...
P8 remains no-auth/local/mock. It does not add OIDC, SSO, login, password auth, session UI, browser mutation forms, production credential material, hosted SaaS operations, Kubernetes/cloud/database mutation, unrestricted shell execution, or live provider mutation.
