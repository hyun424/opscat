...
class SentryReadOnlyConnector:
    ...
    def call(self, request: ConnectorCallRequest) -> ConnectorCallResult:
        ...
        if _provider_mode(request.payload) == _REAL_MODE:
            ...
            if not token:
                return _provider_error_result(request.capability, "missing_secret", "missing credential: sentry.token")
    ...
    def _health(self, request: ConnectorCallRequest) -> ConnectorCallResult:
        ...
        if mode != _REAL_MODE:
            return ConnectorCallResult(
                ...
                ok=True,
                read_only=True,
                evidence_summary="Sentry fixture mode is healthy; no credential or network call required.",
                output={"provider": "sentry-fixture", "mode": _FIXTURE_MODE, "health_state": "fixture_ok", "network_attempted": False},
            )
        token = _optional_text(request.payload.get(_AUTH_TOKEN_FIELD))
        if not token:
        ...
        simulation = _optional_text(request.payload.get("simulate_health"))
        if simulation == "rate_limited":
            return _health_failure("rate_limited", "sentry provider rate limited health check", retry_after=_optional_text(request.payload.get("retry_after")) or "60")
        if simulation == "provider_error":
            return _health_failure("provider_error", "sentry provider health check failed")
        return ConnectorCallResult(
            connector_id=self.connector_id,
            capability=request.capability,
            ok=True,
            read_only=True,
            evidence_summary="Sentry real-provider configuration passed local validation; live reads remain opt-in and bounded.",
            output={"provider": "sentry", "mode": _REAL_MODE, "health_state": "configured", "network_attempted": False},
        )
