...
def test_incident_memory_retrieves_similar_successes_and_failed_warnings() -> None:
    ...
    memory.add(
        IncidentMemoryRecord(
            id="old-ok", service="worker", environment="staging", fingerprint="queue", root_cause="queue backlog", runbook="restart", action_type="mock.execute_restart_worker", outcome="resolved"
        )
    )
    memory.add(
        IncidentMemoryRecord(
            id="old-fail", service="worker", environment="staging", fingerprint="queue", root_cause="queue backlog", runbook="restart", action_type="mock.execute_restart_worker", outcome="failed"
        )
    )
